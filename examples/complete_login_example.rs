use rshioaji::{SecurityType, Shioaji};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 創建合約下載回調函數
    let _contracts_callback = Arc::new(|security_type: SecurityType| {
    ...
    if let Some(stock_account) = client.get_default_stock_account_ref().await {
        println!(
            "   🏦 預設股票帳戶引用：{}",
            stock_account.account.account_id
        );
    }
    ...
    if let Some(futopt_account) = client.get_default_futopt_account_ref().await {
        println!(
            "   🔮 預設期貨帳戶引用：{}",
            futopt_account.account.account_id
        );
    }
