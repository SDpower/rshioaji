use rshioaji::{BidAskFOPv1, BidAskSTKv1, Exchange, QuoteSTKv1, Shioaji, TickFOPv1, TickSTKv1};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client
        .on_tick_stk_v1(
            |exchange: Exchange, tick: TickSTKv1| {
                println!(
                    "📈 股票 Tick 資料: {} - {} @ {}",
    ...
    client
        .on_tick_fop_v1(
            |exchange: Exchange, tick: TickFOPv1| {
                println!(
                    "📊 期貨 Tick 資料: {} - {} @ {}",
                    exchange, tick.code, tick.close
                );
    ...
    client
        .on_bidask_stk_v1(
            |exchange: Exchange, bidask: BidAskSTKv1| {
                println!(
                    "💹 股票 BidAsk: {} - {} Bid: {:?} Ask: {:?}",
    ...
    client
        .on_quote_stk_v1(
            |exchange: Exchange, quote: QuoteSTKv1| {
                println!(
                    "📊 股票 Quote: {} - {} @ {} Vol: {}",
    ...
    client
        .on_quote(|topic: String, data: serde_json::Value| {
    ...
    client
        .on_event(
            |resp_code: i32, event_code: i32, info: String, event: String| {
                println!(
                    "⚡ 系統事件: {} {} - {} {}",
