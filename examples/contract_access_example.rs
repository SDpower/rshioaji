...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 嘗試在未登入時下單（這會失敗）
    let test_order = Order::new(Action::Buy, 500.0, 1000, OrderType::ROD, StockPriceType::LMT);
    
    match client.place_order(test_stock.contract.clone(), test_order).await {
    ...
    // 現在可以正確存取合約
    let new_order = Order::new(Action::Buy, 500.0, 1000, OrderType::ROD, StockPriceType::LMT);
    match client.place_order(test_stock.contract.clone(), new_order).await {
        Ok(trade) => {
            println!("   ✅ 下單成功！");
            println!("      📝 get_system_contract 現在能從 api.Contracts.Stocks[\"2330\"] 取得真實合約");
