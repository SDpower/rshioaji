use rshioaji::{Action, EnvironmentConfig, OrderType, Shioaji};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client
        .on_tick_stk_v1(
            |exchange, tick| {
                println!(
                    "📈 Exchange: {:?}, TickSTK: {{ code: {}, close: {}, volume: {} }}",
    ...
    client
        .on_tick_fop_v1(
            |exchange, tick| {
                println!(
                    "📊 Exchange: {:?}, TickFOP: {{ code: {}, close: {}, volume: {} }}",
                    exchange, tick.code, tick.close, tick.volume
                );
            },
            false,
    ...
    // 從環境變數讀取真實 API 憑證
    let api_key = std::env::var("SHIOAJI_API_KEY").unwrap_or_else(|_| {
        println!("⚠️  未找到 SHIOAJI_API_KEY 環境變數，使用測試憑證");
        "TEST_API_KEY_12345".to_string()
    });
    let secret_key = std::env::var("SHIOAJI_SECRET_KEY").unwrap_or_else(|_| {
        println!("⚠️  未找到 SHIOAJI_SECRET_KEY 環境變數，使用測試憑證");
        "TEST_SECRET_KEY_67890".to_string()
    });

    // 檢查是否使用真實憑證
    let is_real_credentials =
        api_key != "TEST_API_KEY_12345" && secret_key != "TEST_SECRET_KEY_67890";
    ...
    match client
        .get_kbars(tsmc.contract.clone(), &start_date, &end_date)
        .await
    {
        Ok(kbars) => {
