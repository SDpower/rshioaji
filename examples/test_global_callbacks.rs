use rshioaji::types::{BidAskFOPv1, BidAskSTKv1, QuoteSTKv1, TickFOPv1, TickSTKv1};
use rshioaji::{Exchange, Shioaji};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client
        .on_quote(|topic, data| {
            println!(
                "🔄 Generic quote callback triggered: {} - {:?}",
                topic, data
            );
        })
        .await?;
    ...
    client
        .trigger_tick_stk_callbacks(Exchange::TSE, tick_stk)
        .await?;
    ...
    client
        .trigger_tick_fop_callbacks(Exchange::TAIFEX, tick_fop)
        .await?;
    ...
    client
        .trigger_bidask_stk_callbacks(Exchange::TSE, bidask_stk)
        .await?;
    ...
    client
        .trigger_bidask_fop_callbacks(Exchange::TAIFEX, bidask_fop)
        .await?;
    ...
    client
        .trigger_quote_stk_callbacks(Exchange::TSE, quote_stk)
        .await?;
    ...
    client
        .trigger_quote_callbacks("market_data".to_string(), quote_data)
        .await?;
