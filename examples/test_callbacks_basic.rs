...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let secret_key = std::env::var("SHIOAJI_SECRET_KEY").unwrap_or_else(|_| "TEST_SECRET_KEY".to_string());
    ...
    match client.on_tick_stk_v1(|exchange, tick| {
        println!("📈 [Rust] 股票 Tick 收到: {:?} - {} @ {}", exchange, tick.code, tick.close);
    }, false).await {
    ...
    match client.on_tick_fop_v1(|exchange, tick| {
        println!("📊 [Rust] 期貨 Tick 收到: {:?} - {} @ {}", exchange, tick.code, tick.close);
    }, false).await {
    ...
    match client.on_bidask_stk_v1(|exchange, bidask| {
    ...
    match client.on_bidask_fop_v1(|exchange, bidask| {
    ...
    match client.on_quote_stk_v1(|exchange, quote| {
    ...
    // 6. 註冊通用報價回調
    println!("\n6️⃣ 註冊 on_quote 回調...");
    match client.on_quote(|topic, data| {
    ...
    // 7. 註冊系統事件回調
    println!("\n7️⃣ 註冊 on_event 回調...");
    match client.on_event(|resp_code, event_code, info, event| {
        println!("⚡ [Rust] 系統事件收到: {} {} - {} {}", resp_code, event_code, info, event);
    }).await {
