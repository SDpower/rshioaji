...
use crate::callbacks::{ContractCallback, EventHandlers};
use crate::error::{Error, Result};
use crate::types::*;
use crate::utils::{
    check_contract_cache, clear_outdated_contract_cache_default, get_contracts_filename,
    new_contracts,
};

// Type aliases for complex callback types
type TickSTKCallback = Arc<dyn Fn(Exchange, TickSTKv1) + Send + Sync>;
type TickFOPCallback = Arc<dyn Fn(Exchange, TickFOPv1) + Send + Sync>;
type BidAskSTKCallback = Arc<dyn Fn(Exchange, BidAskSTKv1) + Send + Sync>;
type BidAskFOPCallback = Arc<dyn Fn(Exchange, BidAskFOPv1) + Send + Sync>;
type QuoteSTKCallback = Arc<dyn Fn(Exchange, QuoteSTKv1) + Send + Sync>;
type QuoteCallback = Arc<dyn Fn(String, serde_json::Value) + Send + Sync>;
type EventCallback = Arc<dyn Fn(i32, i32, String, String) + Send + Sync>;
type SessionDownCallback = Arc<dyn Fn() + Send + Sync>;
...
pub struct Shioaji {
    ...
    // === 全域 Callback 儲存機制 ===
    /// 股票 Tick 回調函數
    tick_stk_callbacks: Arc<Mutex<Vec<TickSTKCallback>>>,
    /// 期貨/選擇權 Tick 回調函數
    tick_fop_callbacks: Arc<Mutex<Vec<TickFOPCallback>>>,
    /// 股票 BidAsk 回調函數
    bidask_stk_callbacks: Arc<Mutex<Vec<BidAskSTKCallback>>>,
    /// 期貨/選擇權 BidAsk 回調函數
    bidask_fop_callbacks: Arc<Mutex<Vec<BidAskFOPCallback>>>,
    /// 股票 Quote 回調函數
    quote_stk_callbacks: Arc<Mutex<Vec<QuoteSTKCallback>>>,
    /// 通用 Quote 回調函數
    quote_callbacks: Arc<Mutex<Vec<QuoteCallback>>>,
    /// 系統事件回調函數
    event_callbacks: Arc<Mutex<Vec<EventCallback>>>,
    /// 連線中斷回調函數
    session_down_callbacks: Arc<Mutex<Vec<SessionDownCallback>>>,
}
...
impl Shioaji {
    ...
    pub fn new_with_options(
        ...
        let simu_to_stag = simulation
            && std::env::var("SHIOAJI_SIMU_TO_STAG")
                .unwrap_or_default()
                .parse::<bool>()
                .unwrap_or(false);
        ...
        // Enforce single instance connection
        {
            ...
            if *instance_lock {
                return Err(Error::Initialization(
                    "Shioaji instance already exists. Only one connection allowed.".to_string(),
                ));
            }
            *instance_lock = true;
        }
    ...
    pub async fn login(
        ...
        log::info!(
            "📊 Fetch contracts: {}, timeout: {}ms",
            fetch_contract,
            contracts_timeout
        );
        log::info!(
            "📊 Subscribe trade: {}, receive window: {}ms",
            subscribe_trade,
            receive_window
        );
        log::info!(
            "📊 Simulation mode: {}, simu_to_stag: {}",
            self.simulation,
            self._simu_to_stag
        );
        ...
        if api_key.is_empty() || secret_key.is_empty() {
            return Err(Error::Authentication(
                "API key or secret key is empty".to_string(),
            ));
        }
        ...
        // Check if already logged in (prevent multiple connections)
        {
            ...
            if *logged_in {
                ...
                return Err(Error::Authentication(
                    "Already logged in. Multiple connections not allowed.".to_string(),
                ));
            }
        }

        // 直接使用真實 shioaji 進行登入 (移除不必要的包裝層)
        let (accounts, contract_download, person_id, api_instance) =
            Python::with_gil(|py| -> Result<(Vec<Account>, bool, String, PyObject)> {
                log::info!("🌟 Creating system shioaji instance for real login...");

                // 導入 shioaji 模組
                let shioaji_module = py
                    .import("shioaji")
                ...
                let api = if self.vpn {
                    ...
                    log::info!(
                        "🏢 Creating Shioaji instance in {} mode...",
                        if self._simu_to_stag {
                            "simulation-to-staging"
                        } else {
                            "standard"
                        }
                    );
                ...
                let login_result = api
                    .call_method("login", (api_key, secret_key), Some(kwargs))
                ...
                if let Ok(accounts_list) = login_result.downcast::<pyo3::types::PyList>() {
                    log::info!(
                        "📊 Processing {} accounts from login result...",
                        accounts_list.len()
                    );
                    ...
                    return Err(Error::Authentication(
                        "Unexpected login result format - expected List[Account]".to_string(),
                    ));
                }
                ...
                let person_id = if let Ok(person_id_py) = api.getattr("person_id") {
                    person_id_py
                        .extract::<String>()
                        .unwrap_or_else(|_| format!("user_{}", chrono::Utc::now().timestamp()))
                } else {
                    format!("user_{}", chrono::Utc::now().timestamp())
                };

                log::info!(
                    "✅ Real system shioaji login completed with {} accounts",
                    accounts.len()
                );
                ...
                if accounts.is_empty() {
                    return Err(Error::Authentication(
                        "No accounts returned from system shioaji login".to_string(),
                    ));
                }
        ...
        if fetch_contract {
            log::info!(
                "📊 Real shioaji login automatically downloads contracts (fetch_contract={})",
                fetch_contract
            );
        ...
        if let Err(e) = self.perform_system_setup_callbacks(&api_instance).await {
            log::warn!(
                "⚠️ Failed to setup callbacks: {}, continuing without callbacks",
                e
            );
        } else {
        ...
        log::info!(
            "✅ Login completed with {} accounts using system shioaji",
            accounts.len()
        );
        log::info!("   📋 Person ID: {}", person_id);
    ...
    async fn load_contracts_from_instance(&self, instance: &PyObject) -> Result<()> {
        Python::with_gil(|py| -> Result<()> {
            ...
            let contracts_obj = instance.getattr(py, "Contracts").map_err(|e| {
                Error::System(format!(
                    "Failed to get Contracts from shioaji instance: {}",
                    e
                ))
            })?;

            // 取得各類型合約
            let stocks = contracts_obj
                .getattr(py, "Stocks")
                .map_err(|e| Error::System(format!("Failed to get Stocks: {}", e)))?;
            let futures = contracts_obj
                .getattr(py, "Futures")
                .map_err(|e| Error::System(format!("Failed to get Futures: {}", e)))?;
            let options = contracts_obj
                .getattr(py, "Options")
                .map_err(|e| Error::System(format!("Failed to get Options: {}", e)))?;
            let indices = contracts_obj
                .getattr(py, "Indexs") // 注意：原始 API 是 "Indexs" 不是 "Indices"
                .map_err(|e| Error::System(format!("Failed to get Indexs: {}", e)))?;

            // 計算股票合約數量 (遍歷群組：OES, OTC, TSE) 並記錄細分數量
            let (stocks_len, stocks_tse, stocks_otc, stocks_oes) =
                if let Ok(keys_generator) = stocks.call_method0(py, "keys") {
                    // 將 generator 轉換為 list
                    if let Ok(keys_list) = py
                        .import("builtins")?
                        .call_method1("list", (keys_generator,))
                    {
                        if let Ok(keys) = keys_list.extract::<Vec<String>>() {
                            ...
                            for key in keys {
                                if let Ok(group) = stocks.getattr(py, key.as_str()) {
                                    ...
                                    if let Ok(py_list) =
                                        py.import("builtins")?.call_method1("list", (group,))
                                    {
                                        if let Ok(contracts_list) =
                                            py_list.downcast::<pyo3::types::PyList>()
                                        {
    ...
    fn parse_account_from_python(&self, _py: Python, account_py: &PyAny) -> Result<Account> {
        // 提取帳戶資訊
        let broker_id = account_py
            .getattr("broker_id")?
            .extract::<String>()
            .unwrap_or("9A95".to_string());
        let account_id = account_py
            .getattr("account_id")?
            .extract::<String>()
            .unwrap_or("Unknown".to_string());
        let username = account_py
            .getattr("username")?
            .extract::<String>()
            .unwrap_or("Unknown".to_string());
        let signed = account_py
            .getattr("signed")?
            .extract::<bool>()
            .unwrap_or(false);

        // 確定帳戶類型
        let account_type_str = account_py
            .getattr("account_type")?
            .extract::<String>()
    ...
    async fn setup_error_tracking(&self, person_id: &str) -> Result<()> {
        ...
        let error_tracking_status = self
            .get_solace_error_tracking(person_id)
            .await
            .unwrap_or(true);
        log::info!(
            "📊 Error tracking status from Solace: {}",
            error_tracking_status
        );
        ...
        #[cfg(feature = "sentry")]
        {
            if config.log_sentry && !self.simulation && error_tracking_status {
                ...
                log::info!(
                    "🔍 Sentry user context configured for person_id: {}",
                    person_id
                );
            }
        }
        ...
        log::info!(
            "✅ Error tracking setup completed - enabled: {}",
            error_tracking_status
        );
        Ok(())
    }
    ...
    async fn get_solace_error_tracking(&self, person_id: &str) -> Result<bool> {
        Python::with_gil(|py| -> Result<bool> {
            log::info!(
                "🔍 Getting error tracking status from Solace for person_id: {}",
                person_id
            );
    ...
    fn try_real_solace_error_tracking(&self, py: Python, person_id: &str) -> Result<bool> {
        ...
        log::info!(
            "✅ Real Solace error_tracking result: {}",
            error_tracking_status
        );
    ...
    async fn setup_default_account_references(&self, instance: &PyObject) -> Result<()> {
        Python::with_gil(|py| -> Result<()> {
            ...
            match self.try_setup_real_default_accounts(py, instance) {
                ...
                Err(e) => {
                    log::warn!(
                        "⚠️ Real default account setup failed: {}, using fallback",
                        e
                    );
                }
            }
    ...
    fn try_setup_real_default_accounts(&self, py: Python, instance: &PyObject) -> Result<()> {
        ...
        if let Ok(solace) = instance.getattr(py, "_solace") {
            ...
            Err(Error::Initialization(
                "No _solace object found in shioaji instance".to_string(),
            ))
        }
    }
    ...
    fn extract_system_account(&self, py: Python, account_obj: &PyObject) -> Result<Account> {
        ...
        let signed = account_obj
            .getattr(py, "signed")
            .and_then(|v| v.extract::<bool>(py))
            .unwrap_or(true);

        let account_type_obj = account_obj
    ...
    pub async fn fetch_contracts(
        ...
        let result = self
            .fetch_contracts_internal(contract_download, contracts_timeout, _contracts_cb)
            .await;
            ...
            Ok(counts) => {
                ...
                if total < 1000 {
                    ...
                    if total_f > 0.0 {
                        ...
                        println!(
                            "   股票:   {:>6.1}%",
                            (counts.stocks as f64 / total_f) * 100.0
                        );
                        println!(
                            "   期貨:   {:>6.1}%",
                            (counts.futures as f64 / total_f) * 100.0
                        );
                        println!(
                            "   選擇權: {:>6.1}%",
                            (counts.options as f64 / total_f) * 100.0
                        );
                        println!(
                            "   指數:   {:>6.1}%",
                            (counts.indices as f64 / total_f) * 100.0
                        );
    ...
    async fn fetch_contracts_internal(
        ...
        log::info!(
            "📊 Starting contract fetch - download: {}, timeout: {}ms",
            contract_download,
            contracts_timeout
        );
        ...
        if contract_download || !todayfile_exist {
            ...
            log::info!(
                "🌐 呼叫 _solace.fetch_all_contract (強制下載: {}, 快取存在: {})",
                contract_download,
                todayfile_exist
            );
            self.call_solace_fetch_all_contract(contracts_timeout, _contracts_cb)
                .await
        } else {
            ...
            if contracts_status == FetchStatus::Unfetch {
                ...
                match crate::utils::load_contracts_file() {
                    Ok(Some(loaded_contracts)) => {
                        ...
                        log::info!(
                            "   📊 合約數量: 股票 {} (TSE: {}, OTC: {}, OES: {})",
                            loaded_contracts.counts.stocks,
                            loaded_contracts.counts.stocks_tse,
                            loaded_contracts.counts.stocks_otc,
                            loaded_contracts.counts.stocks_oes
                        );
                        log::info!(
                            "              期貨 {} ({} 組), 選擇權 {} ({} 組)",
                            loaded_contracts.counts.futures,
                            loaded_contracts.counts.futures_groups,
                            loaded_contracts.counts.options,
                            loaded_contracts.counts.options_groups
                        );
                        log::info!(
                            "              指數 {} (OTC: {}, TAIFEX: {}, TSE: {})",
                            loaded_contracts.counts.indices,
                            loaded_contracts.counts.indices_otc,
                            loaded_contracts.counts.indices_taifex,
                            loaded_contracts.counts.indices_tse
                        );
                    ...
                    Ok(None) => {
                        ...
                        log::warn!(
                            "⚠️ 快取檔案 contracts-{}.pkl 為空，呼叫 _solace.fetch_all_contract",
                            version
                        );
                        self.call_solace_fetch_all_contract(contracts_timeout, _contracts_cb)
                            .await
    ...
    async fn call_solace_fetch_all_contract(
        ...
        log::info!(
            "🌐 執行 self._solace.fetch_all_contract() 下載合約 (timeout: {}ms)",
            contracts_timeout
        );
        ...
        // 檢查登入狀態
        {
            ...
            if !*logged_in {
                return Err(Error::Authentication(
                    "Must login first before fetching contracts. Please call login() method."
                        .to_string(),
                ));
            }
        }
        ...
        let downloaded_contracts = {
            ...
            Python::with_gil(|py| -> Result<Contracts> {
                // 嘗試使用已登入的實例下載合約
                match self.call_logged_in_fetch_contracts(
                    py,
                    contracts_timeout,
                    _contracts_cb.as_deref(),
                ) {
                    Ok(contracts) => {
        ...
        log::info!(
            "✅ 完整 fetch_all_contract 流程完成: 股票 {} (TSE: {}, OTC: {}, OES: {})",
            downloaded_contracts.counts.stocks,
            downloaded_contracts.counts.stocks_tse,
            downloaded_contracts.counts.stocks_otc,
            downloaded_contracts.counts.stocks_oes
        );
        log::info!(
            "   期貨 {} ({} 組), 選擇權 {} ({} 組), 指數 {} (OTC: {}, TAIFEX: {}, TSE: {})",
            downloaded_contracts.counts.futures,
            downloaded_contracts.counts.futures_groups,
            downloaded_contracts.counts.options,
            downloaded_contracts.counts.options_groups,
            downloaded_contracts.counts.indices,
            downloaded_contracts.counts.indices_otc,
            downloaded_contracts.counts.indices_taifex,
            downloaded_contracts.counts.indices_tse
        );
    ...
    fn call_logged_in_fetch_contracts(
            ...
            if let Ok(Some(type_value)) = dict.get_item("type") {
                if let Ok(instance_type) = type_value.extract::<String>() {
                    ...
                    if instance_type == "SolaceProxy" || instance_type == "SystemShioajiProxy" {
                        ...
                        return self.call_proxy_fetch_contracts(
                            py,
                            contracts_timeout,
                            _contracts_cb,
                        );
        ...
        match instance.getattr(py, "type") {
            Ok(type_attr) => match type_attr.extract::<String>(py) {
                Ok(instance_type) => {
        ...
        // 檢查 solace 是否已登入
        let logged_in = solace
            .getattr(py, "logged_in")
            .and_then(|attr| attr.extract::<bool>(py))
            .unwrap_or(false);

        if !logged_in {
            return Err(Error::Authentication(
                "_solace instance not logged in".to_string(),
            ));
        }
        ...
        log::info!(
            "📡 呼叫 _solace.fetch_all_contract(timeout={}ms)...",
            contracts_timeout
        );
        ...
        if _contracts_cb.is_some() {
            ...
            solace
                .call_method1(py, "fetch_all_contract", (contracts_timeout, py_callback))
                .map_err(|e| Error::System(format!("fetch_all_contract failed: {}", e)))?;
        } else {
            // 沒有回調函數的情況 - 使用 None
            let py_none = pyo3::types::PyNone::get(py).to_object(py);
            solace
                .call_method1(py, "fetch_all_contract", (contracts_timeout, py_none))
                .map_err(|e| Error::System(format!("fetch_all_contract failed: {}", e)))?;
        }
        ...
        // 取得下載的合約資料
        log::info!("📋 取得下載的合約資料...");
        let solace_contracts = solace
            .getattr(py, "Contracts")
            .map_err(|e| Error::System(format!("Cannot access Contracts: {}", e)))?;

        // 轉換為 Rust 結構
        let contracts =
            self.convert_python_contracts_to_rust(py, solace_contracts.to_object(py))?;

        log::info!("✅ 使用已登入實例下載合約完成");
        log::info!(
            "   📊 已下載：股票 {} (TSE: {}, OTC: {}, OES: {})",
            contracts.counts.stocks,
            contracts.counts.stocks_tse,
            contracts.counts.stocks_otc,
            contracts.counts.stocks_oes
        );
        log::info!(
            "          期貨 {} ({} 組), 選擇權 {} ({} 組)",
            contracts.counts.futures,
            contracts.counts.futures_groups,
            contracts.counts.options,
            contracts.counts.options_groups
        );
        log::info!(
            "          指數 {} (OTC: {}, TAIFEX: {}, TSE: {})",
            contracts.counts.indices,
            contracts.counts.indices_otc,
            contracts.counts.indices_taifex,
            contracts.counts.indices_tse
        );
...
"#,
            simulation_str, contracts_timeout
        );
    ...
    fn load_proxy_contract_stats(&self) -> Result<Contracts> {
        // 創建基本的合約資料結構
        let cache_dir =
            std::path::Path::new(&std::env::var("HOME").unwrap_or_default()).join(".shioaji");
        let cache_file = cache_dir.join("contracts-1.2.5.json");

        if cache_file.exists() {
            if let Ok(content) = std::fs::read_to_string(&cache_file) {
                if let Ok(stats) =
                    serde_json::from_str::<std::collections::HashMap<String, i32>>(&content)
                {
                    ...
                    if stocks_count > 1000 || futures_count > 50 || options_count > 1000 {
                        return Ok(self.create_real_contract_structure(
                            stocks_count,
                            futures_count,
                            options_count,
                            indices_count,
    ...
    /// 根據統計數據創建真實的合約結構
    fn create_real_contract_structure(
        &self,
        stocks: i32,
        futures: i32,
        options: i32,
...
"#;
        ...
        match py.run(inject_code, None, None) {
            ...
            Err(e) => log::warn!(
                "⚠️ Library path setup failed in call_real_solace: {}, continuing anyway",
                e
            ),
        }
        ...
        let _login_result =
            solace.call_method1("token_login", (&api_key, &secret_key, true, 30000))?;
        ...
        // 對應原始 Python: self._solace.fetch_all_contract(contracts_timeout, _contracts_cb)
        log::info!(
            "📡 實際呼叫 _solace.fetch_all_contract(timeout={}ms, callback={:?})",
            contracts_timeout,
            _contracts_cb.is_some()
        );
    ...
    fn convert_python_contracts_to_rust(
        ...
        log::info!(
            "✅ Contract conversion completed: {} stocks, {} futures, {} options",
            contracts.counts.stocks,
            contracts.counts.futures,
            contracts.counts.options
        );
    ...
    pub async fn fetch_contracts_test_mode(&self) -> Result<ContractCounts> {
        ...
        log::info!(
            "✅ 測試模式合約下載完成: 股票 {} (TSE: {}, OTC: {}, OES: {})",
            test_contracts.counts.stocks,
            test_contracts.counts.stocks_tse,
            test_contracts.counts.stocks_otc,
            test_contracts.counts.stocks_oes
        );
        log::info!(
            "   期貨 {} ({} 組), 選擇權 {} ({} 組), 指數 {} (OTC: {}, TAIFEX: {}, TSE: {})",
            test_contracts.counts.futures,
            test_contracts.counts.futures_groups,
            test_contracts.counts.options,
            test_contracts.counts.options_groups,
            test_contracts.counts.indices,
            test_contracts.counts.indices_otc,
            test_contracts.counts.indices_taifex,
            test_contracts.counts.indices_tse
        );
    ...
    pub async fn place_order(&self, contract: Contract, order: Order) -> Result<Trade> {
        log::info!(
            "📊 Placing order using system shioaji for contract: {}",
            contract.base.code
        );
        ...
        // Validate login state
        {
            ...
            if !*logged_in {
                return Err(Error::NotLoggedIn(
                    "Must login before placing orders".to_string(),
                ));
            }
        }
        ...
        let trade = self
            .perform_system_place_order(&instance, contract, order)
            .await?;

        log::info!(
            "✅ Order placed successfully using system shioaji: Order ID {}",
            trade.order_id
        );
        Ok(trade)
    }
    ...
    async fn perform_system_place_order(
        &self,
        instance: &PyObject,
        contract: Contract,
        order: Order,
        ...
        Python::with_gil(|py| -> Result<Trade> {
            ...
            let trade_result = instance
                .call_method(py, "place_order", (py_contract, py_order), None)
                .map_err(|e| {
                    Error::Trading(format!("System shioaji place_order failed: {:?}", e))
                })?;
    ...
    fn get_system_contract(&self, py: Python, contract: &Contract) -> Result<PyObject> {
        ...
        if !logged_in {
            return Err(Error::Authentication(
                "Must login first before accessing contracts. Please call login() method."
                    .to_string(),
            ));
        }
        ...
        let instance = {
            ...
            instance_guard
                .as_ref()
                .ok_or_else(|| Error::Trading("Client not initialized".to_string()))?
                .clone()
        };
        ...
        let py_contract = if let Ok(dict) = contract_collection.downcast::<pyo3::types::PyDict>(py)
        {
            ...
            contract_collection
                .call_method1(py, "__getitem__", (&contract.base.code,))
                .map_err(|e| {
                    Error::Trading(format!(
                        "Contract {} not found in {:?} collection: {:?}",
    ...
    fn create_system_order(&self, py: Python, order: &Order) -> Result<PyObject> {
        ...
        let py_order = order_class
            .call((), Some(order_dict))
    ...
    fn convert_system_trade_result(
        &self,
        py: Python,
        trade_result: &PyObject,
        contract: &Contract,
        order: &Order,
    ) -> Result<Trade> {
        let order_id = trade_result
            .getattr(py, "order_id")
            .and_then(|v| v.extract::<String>(py))
            .unwrap_or_else(|_| format!("order_{}", chrono::Utc::now().timestamp()));

        let seqno = trade_result
            .getattr(py, "seqno")
            .and_then(|v| v.extract::<String>(py))
            .unwrap_or_else(|_| "0".to_string());

        let ordno = trade_result
            .getattr(py, "ordno")
            .and_then(|v| v.extract::<String>(py))
            .unwrap_or_else(|_| "0".to_string());

        let status_obj = trade_result
            .getattr(py, "status")
            .unwrap_or_else(|_| py.None());
    ...
    pub async fn subscribe(&self, contract: Contract, quote_type: &str) -> Result<String> {
        log::info!(
            "📊 Subscribing to {} data using system shioaji for {}",
            quote_type,
            contract.base.code
        );
        ...
        // Validate login state
        {
            ...
            if !*logged_in {
                return Err(Error::NotLoggedIn(
                    "Must login before subscribing to market data".to_string(),
                ));
            }
        }
        ...
        let subscription_id = self
            .perform_system_subscribe(&instance, contract, quote_type)
            .await?;

        log::info!(
            "✅ Subscription created using system shioaji: {}",
            subscription_id
        );
        Ok(subscription_id)
    }
    ...
    /// Perform system shioaji subscription
    async fn perform_system_subscribe(
        &self,
        instance: &PyObject,
        contract: Contract,
        quote_type: &str,
        ...
        Python::with_gil(|py| -> Result<String> {
            ...
            let contracts = instance.getattr(py, "Contracts").map_err(|e| {
                Error::Subscription(format!("Failed to get Contracts object: {:?}", e))
            })?;
            ...
            .map_err(|e| {
                Error::Subscription(format!("Failed to get contract collection: {:?}", e))
            })?;
            ...
            let python_contract = if contract.base.security_type == SecurityType::Future {
                ...
                log::info!(
                    "📊 Getting futures contract: Contracts.Futures.{}[{}]",
                    exchange_group,
                    contract.base.code
                );
                ...
                let futures_group =
                    contract_collection
                        .getattr(py, exchange_group)
                        .map_err(|e| {
                            Error::Subscription(format!(
                                "Failed to get futures group {}: {:?}",
                                exchange_group, e
                            ))
                        })?;
                ...
                futures_group
                    ...
                    .map_err(|e| {
                        Error::Subscription(format!(
                            ...
                            contract.base.code, exchange_group, e
                        ))
                    })?
            } else {
            ...
            let _result = quote
                .call_method(py, "subscribe", (python_contract,), Some(kwargs))
                .map_err(|e| Error::Subscription(format!("Quote.subscribe failed: {:?}", e)))?;

            log::info!(
                "✅ Quote.subscribe successful for {} ({})",
                contract.base.code,
                quote_type
            );
    ...
    async fn perform_system_setup_callbacks(&self, instance: &PyObject) -> Result<()> {
        Python::with_gil(|py| -> Result<()> {
            ...
            let tick_stk_callback = pyo3::types::PyCFunction::new_closure(
                py,
                None,
                None,
                move |args, _kwargs| -> PyResult<PyObject> {
                    ...
                    if args.len() >= 2 {
                        ...
                        let exchange = if let Ok(exchange_str) =
                        ...
                        if let Ok(tick_obj) = args.get_item(1) {
                            ...
                            let tick_data = crate::types::TickSTKv1 {
                                ...
                                amount: tick_obj
                                    ...
                                    .and_then(|v| v.extract::<f64>())
                                    .unwrap_or(0.0),
                                total_amount: tick_obj
                                    .getattr("total_amount")
                                    .and_then(|v| v.extract::<f64>())
                                    .unwrap_or(0.0),
                                total_volume: tick_obj
                                    .getattr("total_volume")
                                    .and_then(|v| v.extract::<i64>())
                                    .unwrap_or(0),
                                tick_type: tick_obj
                                    .getattr("tick_type")
                                ...
                                bid_side_total_vol: tick_obj
                                    .getattr("bid_side_total_vol")
                                    .and_then(|v| v.extract::<i64>())
                                    .unwrap_or(0),
                                ask_side_total_vol: tick_obj
                                    .getattr("ask_side_total_vol")
                                    .and_then(|v| v.extract::<i64>())
                                    .unwrap_or(0),
                                bid_side_total_cnt: tick_obj
                                    .getattr("bid_side_total_cnt")
                                    .and_then(|v| v.extract::<i64>())
                                    .unwrap_or(0),
                                ask_side_total_cnt: tick_obj
                                    .getattr("ask_side_total_cnt")
                                    .and_then(|v| v.extract::<i64>())
                                    .unwrap_or(0),
                                suspend: tick_obj
                                    .getattr("suspend")
                            ...
                            if let Ok(callbacks_guard) = tick_stk_callbacks.try_lock() {
                                ...
                                if !callbacks_guard.is_empty() {
                                    log::debug!(
                                        "✅ Triggered {} STK tick callbacks for {}",
                                        callbacks_guard.len(),
                                        tick_data.code
                                    );
                                }
                            ...
                            log::warn!(
                                "Failed to extract tick object from STK tick callback args: {:?}",
                                args
                            );
                        }
                    } else {
            ...
            let tick_fop_callback = pyo3::types::PyCFunction::new_closure(
                py,
                None,
                None,
                move |args, _kwargs| -> PyResult<PyObject> {
                        ...
                        if let Ok(tick_obj) = args.get_item(1) {
                            ...
                            let tick_data = crate::types::TickFOPv1 {
                                ...
                                avg_price: tick_obj
                                    ...
                                    .unwrap_or(0.0),
                                datetime: chrono::Utc::now(), // Use current time as default
                                underlying_price: tick_obj
                                    .getattr("underlying_price")
                                    .and_then(|v| v.extract::<f64>())
                                    .unwrap_or(0.0),
                                bid_side_total_vol: tick_obj
                                    .getattr("bid_side_total_vol")
                            ...
                            if let Ok(callbacks_guard) = tick_fop_callbacks.try_lock() {
                                ...
                                if !callbacks_guard.is_empty() {
                                    log::debug!(
                                        "✅ Triggered {} FOP tick callbacks for {}",
                                        callbacks_guard.len(),
                                        tick_data.code
                                    );
                                }
                            ...
                            log::warn!(
                                "Failed to extract tick object from FOP tick callback args: {:?}",
                                args
                            );
                        }
                    } else {
            ...
            let bidask_stk_callback = pyo3::types::PyCFunction::new_closure(
                py,
                None,
                None,
            ...
            let bidask_fop_callback = pyo3::types::PyCFunction::new_closure(
                py,
                None,
                None,
                |args, _kwargs| -> PyResult<PyObject> {
            ...
            let quote_stk_callback = pyo3::types::PyCFunction::new_closure(
                py,
                None,
                None,
                |args, _kwargs| -> PyResult<PyObject> {
            ...
            let event_callback = pyo3::types::PyCFunction::new_closure(
                py,
                None,
                None,
                move |args, _kwargs| -> PyResult<PyObject> {
                    // Extract parameters with proper error handling
                    let resp_code: i32 = args
                        .get_item(0)
                        .and_then(|item| item.extract())
                        .unwrap_or(0);
                    let event_code: i32 = args
                        .get_item(1)
                        .and_then(|item| item.extract())
                        .unwrap_or(0);
                    let info: String = args
                        .get_item(2)
                        .and_then(|item| item.extract())
    ...
    pub fn create_option(
        &self,
        code: &str,
        option_right: crate::types::OptionRight,
        strike_price: f64,
    ...
    pub async fn get_kbars(&self, contract: Contract, start: &str, end: &str) -> Result<Vec<Kbar>> {
        log::info!(
            "📊 Fetching K-bars using system shioaji for {} from {} to {}",
            contract.base.code,
            start,
            end
        );
        ...
        // Validate login state
        {
            ...
            if !*logged_in {
                return Err(Error::NotLoggedIn(
                    "Must login before fetching K-bars".to_string(),
                ));
            }
        }
        ...
        let kbars = self
            .perform_system_get_kbars(&instance, contract, start, end)
            .await?;
    ...
    async fn perform_system_get_kbars(
        &self,
        instance: &PyObject,
        contract: Contract,
        start: &str,
        ...
        Python::with_gil(|py| -> Result<Vec<Kbar>> {
            ...
            let kbars_result = instance
                .call_method(py, "kbars", (py_contract,), Some(kwargs))
