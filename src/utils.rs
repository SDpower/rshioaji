...
impl EnvironmentConfig {
    ...
    pub fn from_env() -> Self {
        ...
        let sentry_uri = env::var("SENTRY_URI").unwrap_or_else(|_| DEFAULT_SENTRY_URI.to_string());
        ...
        let sj_log_path =
            env::var("SJ_LOG_PATH").unwrap_or_else(|_| DEFAULT_SJ_LOG_PATH.to_string());
...
pub fn set_error_tracking(simulation: bool, error_tracking: bool, config: &EnvironmentConfig) {
    if config.log_sentry && !simulation && error_tracking {
        ...
        log::debug!(
            "錯誤追蹤系統未啟用 (simulation={}, error_tracking={}, log_sentry={})",
            simulation,
            error_tracking,
            config.log_sentry
        );
...
/// ```
pub fn clear_outdated_contract_cache<P: AsRef<Path>>(
    contract_path: P,
    keep_days: u64,
) -> Result<(), Box<dyn std::error::Error>> {
    let contract_path = contract_path.as_ref();
    let contract_dir = contract_path.parent().ok_or("無法取得合約目錄")?;
...
pub fn clear_outdated_contract_cache_default<P: AsRef<Path>>(
    contract_path: P,
) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn create_shared_folder() -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
    let home_dir = dirs::home_dir().ok_or("無法取得使用者主目錄")?;
...
pub fn create_default_test_contracts() -> Contracts {
    ...
    // 添加一些測試期貨合約
    let test_futures = vec!["TXFA4", "TXFB4", "TXFC4"];
