use clap::Parser;
use log::info;
use rshioaji::{Action, Config, Exchange, OrderType, Shioaji, StockPriceType};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !api_key.is_empty() && !secret_key.is_empty() {
        ...
        if let Some(stock_code) = cli.stock {
            ...
            match client
                .get_kbars(stock.contract.clone(), &start_date, &end_date)
                .await
            {
                Ok(kbars) => {
            ...
            info!(
                "Order would buy {} shares at {} per share",
        ...
        info!("Example usage:");
        info!(
            "  rshioaji-cli --simulation --api-key YOUR_KEY --secret-key YOUR_SECRET --stock 2330"
        );
    }
